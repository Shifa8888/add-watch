...
export default function App() {
  ...
  function handleLogin(event: FormEvent) {
    ...
    if (!loginEmail.trim() || !loginPassword.trim()) {
      setLoginError("Enter your username/email and password to continue.");
      return;
    }
    localStorage.setItem("gemstar-session", "active");
    localStorage.setItem("gemstar-username", loginEmail.trim());
    setIsLoggedIn(true);
    setLoginError("");
  }
  ...
  function submitDeposit(event: FormEvent) {
    ...
    // In real app, this would send screenshot to customer service
    setAccount((current) => ({
      ...current,
    ...
    setWalletModal(null);
    setToast(`Deposit request for ${money(amount)} sent to customer service. Send screenshot to customer service.`);
  }
  ...
  function submitWithdrawal(event: FormEvent) {
    ...
    setAccount((current) => ({
      ...current,
      balance: current.balance - amount,
      activity: [{ id: Date.now(), title: `Withdrawal request to ${withdrawMethod}`, time: "Just now", amount: -amount, type: "withdraw" as const }, ...current.activity].slice(0, 8),
    }));
    setWithdrawAmount("");
    setWalletModal(null);
    setToast(`Withdrawal request for ${money(amount)} sent to customer service via ${withdrawMethod}. Send screenshot to customer service.`);
  }
    ...
    return <main className="min-h-screen bg-[#f6f8f5] p-4 text-slate-950 sm:p-8">
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-2rem)] max-w-6xl items-center justify-center sm:min-h-[calc(100vh-4rem)]">
        <section className="grid w-full overflow-hidden rounded-[2rem] border border-white/80 bg-white/80 shadow-[0_25px_80px_rgba(20,54,41,0.12)] backdrop-blur-xl lg:grid-cols-[1.08fr_0.92fr]">
          ...
          <div className="flex min-h-[620px] items-center p-6 sm:p-12"><div className="mx-auto w-full max-w-sm"><div className="mb-12 flex items-center gap-3 lg:hidden"><BrandMark /> <span className="text-xl font-semibold tracking-tight">GEM STAR</span></div><div className="mb-9"><p className="mb-3 text-sm font-semibold uppercase tracking-[0.16em] text-blue-600">Member Login</p><h2 className="text-3xl font-semibold tracking-[-0.04em] text-slate-950">Welcome to GEM STAR</h2><p className="mt-2 text-sm leading-6 text-slate-500">Enter username/email and password to access your account.</p></div><form className="space-y-5" onSubmit={handleLogin}><label className="block text-sm font-medium text-slate-700">Username or Email<input value={loginEmail} onChange={(event) => setLoginEmail(event.target.value)} type="text" placeholder="yourusername or email" className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3.5 text-sm outline-none transition focus:border-blue-600 focus:ring-4 focus:ring-blue-100" /></label><label className="block text-sm font-medium text-slate-700">Password<input value={loginPassword} onChange={(event) => setLoginPassword(event.target.value)} type="password" placeholder="Enter your password" className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3.5 text-sm outline-none transition focus:border-blue-600 focus:ring-4 focus:ring-blue-100" /></label>{loginError && <p className="text-sm font-medium text-rose-600">{loginError}</p>}<button type="submit" className="group flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-4 py-3.5 text-sm font-semibold text-white shadow-lg shadow-blue-950/15 transition hover:from-blue-700 hover:to-indigo-700 active:scale-[0.99]">Login to Dashboard <span className="transition-transform group-hover:translate-x-0.5"><Icon name="arrow-right" size={17} /></span></button></form><p className="mt-6 text-center text-xs leading-5 text-slate-400">Demo: Use any credentials to access the dashboard</p></div></div>
        </section>
      </div>
    </main>;
  }
